...
class TextData:
    def __init__(self, fname, line_process_fn=lambda x: x.strip().lower(),
    ...
    def _build_vocabulary_and_stats(self):
        ...
        for line in f:
            ...
            # tokens = tokens[:self.max_len-1] # don't need cutting when build vocab
    ...
    def _tok_line(self, line):
        ...
        """
        return word_tokenize(self.line_process_fn(line).decode('utf-8'))
    ...
    def make_batch(self, encoded_lines, max_len=None):
        ...
        """
        if not max_len:
            max_len = min(max(map(len, encoded_lines))+1, self.max_len)
